...
def preprocess_example(example, dtype=tf.float32):
    ...
    """
    # Optionally, resize images to a fixed size if needed
    image = tf.image.resize(example['image'], [224, 224])  # Resize to 224x224
    image = dataset_utils.normalize(image, dtype)
...
def get_imagenette_dataset(*,
    ...
    """Returns generators for the Imagenette train, validation, and test set.
    ...
    logging.info('Loading train split of the Imagenette dataset.')
    train_ds, train_ds_info = dataset_utils.load_split_from_tfds(
        'imagenette/320px',  # Specify the version or resolution if needed
        batch_size,
        split='train',
    ...
    logging.info('Loading validation split of the Imagenette dataset.')
    ...
    # Update input_shape and other metadata specific to Imagenette
    input_shape = (-1, 224, 224, 3)  # Adjusted for resized images and 3 channels
    meta_data = {
        'num_classes':
            train_ds_info.features['label'].num_classes,  # Typically 10 for Imagenette
        'input_shape':
            input_shape,
        'num_train_examples':
            dataset_utils.get_num_examples('imagenette/320px', 'train'),
        'num_eval_examples':
            dataset_utils.get_num_examples('imagenette/320px', 'validation'),
        'input_dtype':
