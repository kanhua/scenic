...
def psum_metric_normalizer(
        metric_tuple: Tuple[jnp.ndarray, jnp.ndarray],
        axis_name: Optional[Union[str, Tuple[str, ...]]] = 'batch'
) -> Tuple[jnp.ndarray, jnp.ndarray]:
  """
  Normalizes metrics by performing a parallel sum across devices if axis_name is provided.
  Otherwise, performs a regular sum.

  Args:
      metric_tuple: A tuple containing the metric and its normalizer.
      axis_name: The axis name for parallel summing. If None, perform a regular sum.

  Returns:
      A tuple of (summed_metric, summed_normalizer).
  """
  if axis_name:
    summed_metric = jax.lax.psum(metric_tuple[0], axis_name=axis_name)
    summed_normalizer = jax.lax.psum(metric_tuple[1], axis_name=axis_name)
  else:
    summed_metric = jnp.sum(metric_tuple[0])
    summed_normalizer = jnp.sum(metric_tuple[1])
