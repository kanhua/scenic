...
Any]]:
    ...
    logging.info("image shape: %s", batch['inputs'].shape)
    logging.info("label shape: %s", batch['label'].shape)
    logging.info(batch.keys())

    # batch['inputs'] = jnp.squeeze(batch['inputs'], axis=0)
    # batch['label'] = jnp.squeeze(batch['label'], axis=0)
    ...
    metrics = metrics_fn(logits, batch, axis_name=None)
...
def train(
    ...
    train_step_serial = functools.partial(
        train_step,
        flax_model=model.flax_model,
        loss_fn=model.loss_function,
        lr_fn=lr_fn,
    ...
    eval_step_serial = functools.partial(
        eval_step,
        flax_model=model.flax_model,
        metrics_fn=model.get_metrics_fn('validation'),
        debug=config.debug_eval)
    ...
    for step in range(start_step + 1, total_steps + 1):
        with jax.profiler.StepTraceAnnotation('train', step_num=step):
            ...
            logging.info("performing training step %d", step)
            train_state, t_metrics, t_logs = train_step_serial(train_state, train_batch)
        ...
        if (step % log_eval_steps == 1) or (step == total_steps):
            ...
            with report_progress.timed('eval'):
                ...
                train_state = train_state  # train_utils.sync_model_state_across_replicas(train_state)
                for _ in range(steps_per_eval):
                    eval_batch = next(dataset.valid_iter)
                    e_metrics, _ = eval_step_serial(train_state, eval_batch)
